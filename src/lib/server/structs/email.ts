...
export namespace Email {
    ...
    terminal.log(SENDGRID_EMAIL, SENDGRID_API_KEY);
    ...
    export const createLink = (link: string, expires?: Date) => {
        return attemptAsync(async () => {
            ...
            const protocol = [
                ...
                '1',
                'https',
            ].includes((HTTPS || 'f').toLowerCase()) ? 'https://' : 'http://';
    ...
    if (!SENDGRID_API_KEY || !SENDGRID_EMAIL) {
        terminal.warn('SENDGRID_API_KEY or SENDGRID_EMAIL not found in environment variables. There will be no email functionality');
    }
    ...
    export const send = <T extends keyof E>(config: { 
        ...
        return attemptAsync(async () => {
            ...
            terminal.log('Senging email:', config.type, config.subject, config.to, `attachments:${config.attachments?.length || 0}`);
            const file = await fs.promises.readFile(path.join(
                process.cwd(),
                'private',
                'emails',
                config.type + '.html',
