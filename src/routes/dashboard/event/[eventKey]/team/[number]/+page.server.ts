...
export const load = async (event) => {
	...
	const matchScouting = (
		await Scouting.MatchScouting.database
			.select()
			.from(Scouting.MatchScouting.table)
			.innerJoin(
				FIRST.Matches.table,
				eq(FIRST.Matches.table.id, Scouting.MatchScouting.table.matchId)
			)
			.where(
				and(
					eq(FIRST.Matches.table.eventKey, eventKey),
					eq(Scouting.MatchScouting.table.team, number)
				)
			)
	).map((r) => ({
